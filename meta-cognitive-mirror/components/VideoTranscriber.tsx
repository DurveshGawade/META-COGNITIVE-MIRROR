...
const VideoTranscriber: React.FC<VideoTranscriberProps> = ({ onAriaToast }) => {
  ...
  const [interimSpeaker, setInterimSpeaker] = useState<'USER' | 'SYSTEM' | null>(null);
  ...
  const startLiveAudit = async () => {
    ...
    onAriaToast(`NEURAL_LIVE_UPLINK_${selectedLanguage.toUpperCase()}`);
    liveInterimRef.current = "";
    liveModelInterimRef.current = "";
    setInterimText("");
    setInterimSpeaker(null);
      ...
      sessionPromiseRef.current = gemini.connectLiveTranscription({
        ...
        onmessage: (msg: any) => {
          if (msg.serverContent?.outputTranscription) {
            ...
            // SHOW ONLY SYSTEM VERSION: In translation mode, prioritizing the model output.
            setInterimText(liveModelInterimRef.current);
            setInterimSpeaker('SYSTEM');
          } else if (msg.serverContent?.inputTranscription) {
            const text = msg.serverContent.inputTranscription.text;
            liveInterimRef.current += text;
            
            // If English is selected (transcription mode), show user input as primary.
            // If another language is selected (translation mode), strictly SUPPRESS the user input on screen.
            if (selectedLanguage === 'English') {
               setInterimText(liveInterimRef.current);
          ...
          if (msg.serverContent?.turnComplete) {
            ...
            setTranscription(prev => {
              ...
              // Only push USER segment if in English mode (pure transcription)
              if (finalizedInput && selectedLanguage === 'English') {
                newSegments.push({
                  timestamp: `${timeStr}`,
                  startTime: Date.now(),
                  text: finalizedInput,
              ...
              // ALWAYS push SYSTEM segment if it contains translation
              if (finalizedOutput) {
                newSegments.push({
                  timestamp: `${timeStr}`,
                  startTime: Date.now(),
              ...
              return {
                fullText: (prev?.fullText || "") + " " + finalizedOutput,
  ...
  const seekToTimestamp = (timestamp: string) => {
    ...
    if (cleanTs.includes('USER') || cleanTs.includes('SYSTEM')) return;
  ...
  const filteredSegments = useMemo(() => {
    // DISPLAY ONLY SYSTEM VERSION: strictly filter to SYSTEM (translated) version when not in English mode.
    const segments = (transcription?.segments || []).filter(s => {
      if (selectedLanguage !== 'English') return s.speaker === 'SYSTEM';
      return true;
    });
  ...
  }, [transcription, searchQuery, selectedLanguage]);
          ...
          <div className="flex flex-col items-end gap-6 mb-12">
            ...
            <div className="flex gap-4 bg-white/5 p-2 rounded-2xl border border-white/5 backdrop-blur-xl">
              <div className="flex items-center px-4 gap-3">
                <span className="text-[10px] font-black uppercase text-white/30 tracking-widest">Target Language:</span>
                <select 
              ...
              {!isLiveMode && (
                <div className="flex items-center px-4 gap-3 border-l border-white/10">
                  <span className="text-[10px] font-black uppercase text-white/30 tracking-widest">Protocol:</span>
                  <select 
          ...
          <div className="lg:col-span-4 flex flex-col gap-8">
            ...
            <div className="flex flex-col gap-4">
               <button 
                 ...
                 className={`w-full h-24 rounded-[48px] ${isProcessing && !isLiveActive ? 'bg-white/5 text-emerald-500/50' : isLiveActive ? 'bg-red-600 text-white shadow-[0_0_40px_rgba(220,38,38,0.4)]' : 'bg-emerald-500 text-black'} font-black italic uppercase tracking-[1em] text-xl shadow-[0_0_60px_rgba(16,185,129,0.3)] hover:scale-[1.01] transition-all relative overflow-hidden`}
               >
                 {isProcessing ? (
                    ...
                    {showSummary && transcription && (
                      <div className="grid md:grid-cols-2 gap-8">
                        <div className="p-8 rounded-[32px] bg-white/[0.02] border border-white/5 space-y-4">
                          <h4 className="text-[10px] font-black uppercase tracking-widest text-emerald-500">Executive Synthesis</h4>
                          <p className="text-lg font-light italic leading-relaxed text-white/80">"{transcription.summary}"</p>
                        </div>
                        ...
                        <div className="flex flex-col gap-4">
                          ...
                          <AnimatePresence>
                            {interimText && (
                              ...
                              >
                                 <div className="w-24 shrink-0 flex flex-col gap-2 pt-1">
                                    <span className="text-[11px] font-mono text-emerald-400/60 font-bold uppercase tracking-widest">LIVE_NOW</span>
                                    <span className={`text-[8px] font-black uppercase ${interimSpeaker === 'SYSTEM' ? 'text-cyan-400' : 'text-white/40'}`}>{interimSpeaker}</span>
                                 </div>
                                 <div className={`flex-1 p-5 rounded-2xl border transition-all ${interimSpeaker === 'SYSTEM' ? 'bg-cyan-500/10 border-cyan-500/40 animate-pulse' : 'bg-white/5 border-white/10'}`}>
                                    <p className={`text-sm leading-relaxed ${interimSpeaker === 'SYSTEM' ? 'text-cyan-100 font-bold' : 'text-white/80'}`}>{interimText}</p>
                                    <div className="mt-2 flex gap-1">
                                       <motion.div animate={{ opacity: [0, 1, 0] }} transition={{ repeat: Infinity, duration: 1 }} className="w-1.5 h-1.5 rounded-full bg-current" />
                                       <motion.div animate={{ opacity: [0, 1, 0] }} transition={{ repeat: Infinity, duration: 1, delay: 0.2 }} className="w-1.5 h-1.5 rounded-full bg-current" />
                                    </div>
                                 </div>
                              </motion.div>
                            )}
                          ...
                          {!transcription && !interimText && !isProcessing && (
                            <div className="h-40 flex flex-col items-center justify-center opacity-10 uppercase text-[10px] tracking-[2em] text-center p-20 italic">
